...
const VideoPlayerPage: FC = () => {
   ...
   const { data: courseData, isLoading: courseLoading } = useGetSingleCourseQuery(courseId ?? '', { skip: !courseId });
            ...
            <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-white to-cyan-100 dark:from-slate-900 dark:via-slate-950 dark:to-slate-900 py-24">
               <div className="max-w-6xl mx-auto px-4">
                  <div className="text-center">
                     <h1 className="text-2xl font-bold text-red-500 mb-4">Content not found</h1>
                     <Link href={`/courses/${courseId}`} className="px-4 py-2 bg-cyan-600 text-white rounded hover:bg-cyan-700 transition">
                        Back to Course
                     </Link>
                  </div>
               </div>
                     ...
                     {currentComponent.questions && currentComponent.questions.length > 0 && (
                        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 p-6">
                           <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                              ...
                              Questions & Answers ({currentComponent.questions.length})
                           </h3>
                           <div className="space-y-4">
                              {currentComponent.questions.map((question: any, index: number) => (
                                 <div key={question._id || index} className="border-b border-gray-200 dark:border-slate-700 pb-4 last:border-b-0">
                                    ...
                                    {question.questionReplies && question.questionReplies.length > 0 && (
                                       <div className="ml-11 space-y-3">
                                          {question.questionReplies.map((reply: any, replyIndex: number) => (
                                             <div key={reply._id || replyIndex} className="flex items-start gap-3">
