...
export const createLayout = CatchAsyncError(
   async (req: Request, res: Response, next: NextFunction) => {  
      try {
         ...
         if(type === 'FAQ') {
            const { faq } = req.body;
            const faqItmes = await Promise.all(
            ...
            await LayoutModel.create({type:"FAQ", faq: faqItmes});
         }
         if(type === 'Categories') {
            ...
            await LayoutModel.create({type:"Categories",categories:categoriesList});
         }
         res.status(200).json({
            success: true,
   ...
   async (req: Request, res: Response, next: NextFunction) => {  
      try {
         ...
         if(type === 'FAQ') {
            ...
            await LayoutModel.findByIdAndUpdate(faqData._id,{ type:"FAQ",faq:faqItmes });
         }
         ...
         if(type === 'Categories') {
            ...
            await LayoutModel.findByIdAndUpdate(categoriesData._id,{ type:"Categories",categories:categoriesList });
         } 
